...
const SERVER_ID = 'test-echo-exact'
...
export async function GET(): Promise<NextResponse<TestResult>> {
  ...
  if (!recipient || !secretKey) {
    ...
    return NextResponse.json(warn(
      `x402-Exact skipped — missing env vars: ${missing}`,
  ...
  try {
    ...
    const network = exactAccept.network as string
    log.push(`payTo: ${payTo}`)
    log.push(`maxAmountRequired: ${exactAccept.maxAmountRequired}`)
    log.push(`asset: ${exactAccept.asset}`)
    log.push(`network: ${network}`)

    // Dynamic imports — these are serverExternalPackages
    ...
    if (rpc.Api.isSimulationError(simResult)) {
      ...
      return NextResponse.json(fail(
        `Soroban simulation failed — ensure TEST_STELLAR_SECRET_KEY is funded with USDC on testnet: ${errMsg}`,
    ...
    if (paidResult.isError || isPaymentRequired(paidResult)) {
      ...
      return NextResponse.json(fail(
        'Payment was sent but tool still returned an error — check proxy x402ExactHook and facilitator connectivity',
    ...
    return NextResponse.json(pass(
      `x402-Exact full round-trip succeeded: "${text}"`,
      log,

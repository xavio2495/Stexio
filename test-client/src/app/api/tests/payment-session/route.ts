...
const SERVER_ID = 'test-echo-session'
...
export async function GET(): Promise<NextResponse<TestResult>> {
  ...
  if (!recipient || !secretKey || !contractId) {
    ...
    return NextResponse.json(warn(
      `x402-Session skipped — missing env vars: ${missing}`,
  ...
  try {
    ...
    const { result: firstResult } = await callProxyTool(proxyUrl, SERVER_ID, 'echo', { text: 'session test' })
    ...
    if (!sessionAccept) {
      ...
      return NextResponse.json(fail(
        'No session scheme in accepts[]. Ensure proxy has SESSION_CONTRACT_ID and STELLAR_SERVER_SECRET_KEY set, and TEST_SERVER_STELLAR_ADDRESS in test client matches.',
        log, { accepts }
      ))
    }
    ...
    // Step 3: Sign the session payment
    ...
    const { result: paidResult } = await callProxyTool(
      proxyUrl, SERVER_ID, 'echo', { text: 'session test' },
    ...
    return NextResponse.json(pass(
      `x402-Session full round-trip succeeded: "${text}"`,
      log,
      { response: text },
      Date.now() - t0
    ))
